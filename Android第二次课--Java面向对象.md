...
>
> 所以说那个课上的那个方法其实是可以没必要加static的，下面这个更好理解。
>
...
**封装**：是指利⽤抽象数据类型将数据和基于数据的操作封装在⼀起，使其构成⼀个不可分割的独⽴实体，对外不可见。也可以使⽤上⾯的访问修饰符，限制访问权限.（数据隐藏）
...
简单的说就是，你设计一个类，类本身  对提供需求的人  只管我给 封装的类 需求，然后 人 拿到结果，人 不用去管 封装的类内部  是怎么去操作得到结果的。
